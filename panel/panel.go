...
func (p *Panel) loadCore(panelConfig *Config) *core.Instance {
	...
	if panelConfig.BurstObservatoryConfigPath != "" {
	...
	burstObservatoryConfig, err := coreBurstObservatoryConfig.Build()
	if err != nil {
		log.Panicf("Failed to understand BurstObservatory config  Please check: https://xtls.github.io/config/observatory.html for help: %s", err)
	}
	...
	config := &core.Config{
		App: []*serial.TypedMessage{
			...
			serial.ToTypedMessage(burstObservatoryConfig),
		},
